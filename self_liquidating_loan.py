...
class SelfLiquidatingLoan(db.Model):
    ...
    currency = db.Column(Enum(CurrencyType), default=CurrencyType.USD, nullable=False)
    interest_rate = db.Column(db.Float, nullable=False)  # Stored as percentage (e.g., 5.75)
    term_years = db.Column(db.Integer, default=10, nullable=False)
    renewal_options = db.Column(db.Integer, default=2)  # Number of possible renewals
    renewals_used = db.Column(db.Integer, default=0)  # Number of renewals already used
    ...
    renewal_status = db.Column(Enum(RenewalStatus), default=RenewalStatus.NOT_ELIGIBLE)
    ...
    interest_payment_frequency = db.Column(Enum(InterestPaymentFrequency), default=InterestPaymentFrequency.QUARTERLY)
    ...
    collateral_documents_json = db.Column(db.Text)  # JSON array of document IDs
    ...
    # Foreign Keys
    originating_institution_id = db.Column(db.Integer, db.ForeignKey('financial_institution.id'))
    servicing_institution_id = db.Column(db.Integer, db.ForeignKey('financial_institution.id'))
    ...
    def calculate_interest_payment_amount(self):
        ...
        annual_interest = self.current_principal_balance * (self.interest_rate / 100)
    ...
    def is_eligible_for_renewal(self):
        ...
        return (
            ...
            self.renewals_used < self.renewal_options and
            (self.maturity_date - datetime.utcnow()).days <= 180  # Within 6 months of maturity
        )
...
class LoanCollateral(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    loan_id = db.Column(db.Integer, db.ForeignKey('self_liquidating_loan.id'), nullable=False)
    ...
    receivables_aging = db.Column(db.String(50))  # e.g., "0-30 days", "31-60 days"
...
class LoanPayment(db.Model):
    ...
    liquidation_source = db.Column(db.String(255))  # Source of liquidation funds
...
class LoanRenewal(db.Model):
    ...
    additional_terms_json = db.Column(db.Text)  # JSON with any additional terms
...
class LoanCorrespondentAvailability(db.Model):
    ...
    participation_percentage = db.Column(db.Float)  # % of loan available to this correspondent
    participation_amount = db.Column(db.Float)  # Amount available to this correspondent
